...
const UserProfilesListPage: React.FC = () => {
  ...
  const handleLike = async (profile: any) => {
    ...
    if (result.isMatch && result.matchedProfile) {
      setMatchNotification({
        isOpen: true,
        matchedUser: result.matchedProfile
      });
    }
  };
  ...
  const handleSendGift = (profile: any) => {
    setGiftModal({ isOpen: true, profile });
  };

  const handlePlayGame = (profile: any) => {
    setGameModal({ isOpen: true, profile });
  };

  const handleViewProfile = (profile: any) => {
    setProfileDetail({ isOpen: true, profile });
  };
  ...
  const getGameType = (gameName: string): 'icebreaker' | 'compatibility' | 'trivia' | 'puzzle' | 'wordgame' | 'memory' | 'speed' | 'creative' => {
    ...
    const gameMap: Record<string, 'icebreaker' | 'compatibility' | 'trivia' | 'puzzle' | 'wordgame' | 'memory' | 'speed' | 'creative'> = {
      'Trivia Quiz': 'trivia',
      'LGBTQ+ Trivia': 'trivia',
      'Icebreaker Questions': 'icebreaker', 
      'Icebreaker Chat': 'icebreaker',
      'Compatibility Test': 'compatibility',
      'Love Match Test': 'compatibility',
      'Puzzle Challenge': 'puzzle',
      'Pride Puzzles': 'puzzle',
      'Word Game': 'wordgame',
      'Word Challenge': 'wordgame',
      'Memory Match': 'memory',
      'Speed Round': 'speed',
      'Creative Expression': 'creative'
    };
    return gameMap[gameName] || 'icebreaker';
   };
  const canViewPrivateContent = (profile: any) => {
    if (!profile.privacy_settings) return true;
    
    // If profiles are mutually liked, show private content
    if (isLiked(profile.user_id) && profile.likes_current_user) return true;
    
    // Check privacy level
    switch (profile.privacy_settings.profile_visibility) {
      case 'public': return true;
      case 'matches_only': return isLiked(profile.user_id) && profile.likes_current_user;
